...
async function mainTwo() {
    ...
    try {
        let post1 = await posts.createPost('New York', 'As a software developer intern, you will get the opportunity to learn about various aspects of our trucks and how they correlate to the work that our team does. You will get to see the development work that we do in support of our future truck launches and the production support as issues surface.You will get exposure to Agile Methodology for software development, internal customer interviews and developing tools based on their needs!',
            'Software Engineer', 'Programming and Tech', '#software engineer', 'Remote', '15', 'user1')
        let post2 = await posts.createPost('New York', 'We look for potential candidates who is Major in Computer Science, MIS, or other related technology majors', 'Web Scraper',
            'Sales', '#scrapig', 'Remote', '20', 'user1')
        let post3 = await posts.createPost('New York', 'The primary function of the Database Developer would be database deployments and maintenance. To work with application development team in supporting and enhancing current products as well as development of new products. Optimizing data schemes based on application needs.', 'Database Developer',
            'Programming and Tech', '#database engineer, #java', 'Remote', '25', 'user1')
        let post4 = await posts.createPost('New York', ' looking for a remote based Associate (2-4 yrs experience) UI/UX Visual Designer to turn software into easy-to-use products for our clients. UI/UX Designer responsibilities include gathering user requirements, designing graphic elements and building navigation components. To be successful in this role, you should have experience with design software and wireframe tools.', 'Associate Virtual Designer',
            'Sales', '#design #logo', 'Remote', '15', 'user1')
        let post5 = await posts.createPost('New York', 'Looking for the individual who assists in the design, development, testing, and implementation of software applications Develop test cases, and conduct visual and functional testing based on project requirements Properly document all test cases, bugs, and remediation as required for the project team',
            'Software Engineer', 'Programming and Tech', '#software engineer', 'Remote', '50', 'user1')

        let post6 = await posts.createPost('New Jersey', 'looking for a Software Developer (Entry Level) to support our customer in creating capabilities and providing technical services and subject matter expertise in Mission Command and C2 Maneuver systems. You will provide primary technical support to various radar systems to include AN/TPQ – 53 Radar System.', 'Software Engineer',
            'Programming and Tech', '#software engineer, #nj', 'Remote', '30', 'user2')
        let post7 = await posts.createPost('New Jersey', 'The Senior Software Developer will be tasked with fostering and growing the companys various software solutions and infrastructure. The ideal candidate should be well-versed in customer facing software, with a hunger to learn in technology spaces which they have not been exposed to, as well as open to evolve our technology stack to address market changes.', 'Senior Software Developer',
            'Programming and Tech', '#sde, #nj', 'Remote', '35', 'user2')
        let post8 = await posts.createPost('New Jersey', 'Candidate should collaborate with business and end users to understand the requirement and come up with the delivery plan.Analyze requirement and provide delivery estimates.Coordinate within the technology teams to resolve ongoing data related issues.', 'Software Developer',
            'Programming and Tech', '#software engineer, #nj', 'Hybrid', '40', 'user2')
        let post9 = await posts.createPost('New Jersey', 'Creates, develops, plans, writes, and edits operational, instructional, maintenance, or test procedures for paper, multimedia, web-based publications, and user sites; may produce content for embedded user assistance technology.', 'Content Writer',
            'Writing', '#software engineer, #writer', 'Remote', '20', 'user2')
        let post10 = await posts.createPost('New Jersey', 'The Data Analytics Intern is primarily responsible for assisting other Data Analytics team with data mining, data interpretation, and report generation. Be ready to collaborate in a team environment. They will also be working with others who are passionate about making a positive impact on the industry, building data integration solutions to meet the requirements of the business. Manage data analysis and data integration across various systems.', 'Data Analyst',
            'Sales', '#analyst, #data', 'Hybrid', '15', 'user2')
        ...
        let app1 = await application.createApplication('user1', post1._id.toString(), 'Masters', "2", '126 Manhattan,NJ', "25000")
        let app2 = await application.createApplication('user1', post2._id.toString(), 'Masters', "2", '126 Manhattan,NJ', "25000")
        let app3 = await application.createApplication('user1', post3._id.toString(), 'Masters', "2", '126 Manhattan,NJ', "25000")
        let app4 = await application.createApplication('user1', post4._id.toString(), 'Masters', "2", '126 Manhattan,NJ', "25000")
        let app5 = await application.createApplication('user2', post5._id.toString(), 'Bachelors', "2", '26 Graham st,NJ', "25000")
        let app6 = await application.createApplication('user2', post6._id.toString(), 'Bachelors', "2", '26 Graham st,NJ', "25000")
    } catch (e) {
        console.log(e);
    }
